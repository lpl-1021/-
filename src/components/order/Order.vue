    ...
    <el-card>
      ...
      <el-table :data="orderList" border stripe>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="showBox" icon="el-icon-edit"></el-button>
...
export default {
  ...
  methods: {
    async getOrderList() {
      const { data: res } = await this.$http.orders(this.queryInfo);
    ...
    //
    async showProgress() {
      const { data: res } = await this.$http.orderkuaidi();
      if (res.meta.status !== 200) {
