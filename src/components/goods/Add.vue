...
export default {
  ...
  methods: {
    ...
    handleRemove(file) {
      ...
      // 3.调用数组的splice方法，把图片信息从pics数组中的移出
    ...
    add() {
      this.$refs.addFormRef.validate(async (valid) => {
        ...
        let infos = Object.assign({}, this.addForm);
        infos.goods_cat = this.addForm.goods_cat.join(",");
        // console.log(infos)
        // 处理动态参数
        ...
        // 发送请求，添加商品
        // 商品的名称，必须是唯一的
        const {data:res} = await this.$http.addgoods(infos)
        // console.log(res)
